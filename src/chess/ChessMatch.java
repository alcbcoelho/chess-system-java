...
public class ChessMatch {
    ...
    private List<Piece> piecesOnTheBoard = new ArrayList<>();  // the instantiation can go either here or inside the constructor
    ...
    private ChessPiece king(Color color) {
        ...
        throw new IllegalStateException("There is no " + color + " king on the board");     // this exception isn't supposed to be triggered, so I won't even bother catching it. In case it is triggered, the application will end.
    }
...
//        placeNewPiece('f', 5, new Pawn(board, Color.BLACK));
...
//        //////////////////////////////////////////////////////////
        ...
        placeNewPiece('e', 1, new King(board, Color.WHITE, this));  // auto-reference (this keyword)
    ...
    private Piece makeMove(Position source, Position target) {
        ...
        if (p instanceof King && target.getColumn() == source.getColumn() + 2) {
            var sourceR = new Position(source.getRow(), source.getColumn() + 3);
            var targetR = new Position(source.getRow(), source.getColumn() + 1);
            ChessPiece r = (ChessPiece) board.removePiece(sourceR);
        ...
        if (p instanceof King && target.getColumn() == source.getColumn() - 2) {
            var sourceR = new Position(source.getRow(), source.getColumn() - 4);
            var targetR = new Position(source.getRow(), source.getColumn() - 1);
            ChessPiece r = (ChessPiece) board.removePiece(sourceR);
            board.placePiece(r, targetR);
            r.increaseMoveCount();
        ...
        if (p instanceof Pawn) {
            if (source.getColumn() != target.getColumn() && capturedPiece == null) {    // meaning: has the pawn moved diagonally and to an empty position? If so, that means an en passant has been executed
    ...
    private void undoMove(Position source, Position target, Piece capturedPiece) {
        ...
        if (p instanceof King && target.getColumn() == source.getColumn() + 2) {
            var sourceR = new Position(source.getRow(), source.getColumn() + 3);
            var targetR = new Position(source.getRow(), source.getColumn() + 1);
            ChessPiece r = (ChessPiece) board.removePiece(targetR);
